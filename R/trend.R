...
#' @param transforms Optional named list or character vector overriding the
#'   sampling-scale transform of the base's own parameters (currently only the
#'   weight `w` of the `"lin"` and `"centered"` bases, which defaults to
#'   `"identity"`). Values must be one of `"identity"`, `"exp"` or `"pnorm"`.
#'   For example `transforms = list(w = "exp")` constrains the weight to be
#'   positive, which is what a saturating increase in a parameter such as the
#'   stop-signal `muS` needs (see [make_ssd_trend()]).
...
make_base <- function(target_parameter,
  ...
  # ---- optional override of the base parameter transforms ----
  if (!is.null(transforms)) {
    transforms <- unlist(transforms)
    if (is.null(names(transforms)) || any(names(transforms) == ""))
      stop("transforms must be a named list or character vector, e.g. list(w = \"exp\").")
    unknown <- setdiff(names(transforms), generic_pnames)
    if (length(unknown))
      stop("transforms names not parameters of base '", type, "': ",
           paste(unknown, collapse = ", "),
           if (length(generic_pnames)) paste0(" (available: ", paste(generic_pnames, collapse = ", "), ")")
           else " (this base has no parameters)")
    bad <- !transforms %in% c("identity", "exp", "pnorm")
    if (any(bad))
      stop("transforms must be one of 'identity', 'exp', 'pnorm'; got: ",
           paste(transforms[bad], collapse = ", "))
    generic_transforms[names(transforms)] <- as.character(transforms)
  }
...
make_data_unconditional <- function(data, pars, design, model,
  ...
  # ---- staircase SSDs (stop-signal designs simulated with make_ssd()) ----
  # SSD is NA on staircase stop trials; it is resolved here trial by trial so
  # that parameters (e.g. a trend of muS on SSD) see the realised SSD before
  # the trial is simulated, and the ladder is stepped from the simulated
  # response afterwards (same rules as staircase_function()).
  has_stair <- !is.null(ssd_meta) && !is.null(ssd_meta$specs) && "SSD" %in% includeColumns
  if (has_stair) {
    stair_specs     <- ssd_meta$specs
    stair_base      <- attr(stair_specs, "base_spec")
    stair_groupcols <- ssd_meta$group_cols
    if (is.null(stair_groupcols)) stair_groupcols <- attr(stair_specs, "group_cols")
    stair_UC        <- ssd_meta$UC
    stair_rules     <- ssd_meta$rules
    ladder          <- list()
    stair_spec_for <- function(gid) {
      spec <- stair_specs[[gid]]
      if (is.null(spec)) {
        if (length(stair_specs) == 1L) spec <- stair_specs[[1]] else
          stop("No staircase specification for group '", gid, "'.")
      }
      if (is.null(spec$SSD0)) spec$SSD0 <- stair_base$SSD0
      if (is.null(spec$rules)) spec$rules <- stair_rules
      if (is.null(spec$UC)) spec$UC <- stair_UC
      spec
    }
  }
  ...
  for (subj in subj_levels) {
    ...
    for (j in seq_along(trial_vals)) {
      ...
      # 0. Staircase: resolve this trial's SSD from the group's ladder
      stair_gid <- NULL
      if (has_stair && is.na(dadm_subj_df[["SSD"]][idx_curr[1]])) {
        stair_gid <- if (length(stair_groupcols)) {
          paste(vapply(stair_groupcols, function(g)
            as.character(dadm_subj_df[[g]][idx_curr[1]]), character(1)), collapse = "::")
        } else ".all"
        stair_spec <- stair_spec_for(stair_gid)
        if (is.null(ladder[[stair_gid]])) ladder[[stair_gid]] <- stair_spec$SSD0
        ssd_now <- staircase_clamp(ladder[[stair_gid]], stair_spec)
        ladder[[stair_gid]] <- ssd_now
        dadm_subj_df[["SSD"]][idx_curr] <- ssd_now
      }
      ...
      # 9. Staircase: step the ladder from the simulated response
      if (!is.null(stair_gid)) {
        R_i  <- Rrt[1, "R"]
        rt_i <- if ("rt" %in% colnames(Rrt)) Rrt[1, "rt"] else NA_real_
        label <- if (is.na(R_i)) NA_character_ else as.character(R_i)
        late  <- !is.null(stair_spec$UC) && is.finite(stair_spec$UC) &&
          !is.na(rt_i) && rt_i > stair_spec$UC
        step_dir <- staircase_step_dir(stair_spec, label, stopped = is.na(R_i), late = late)
        ladder[[stair_gid]] <- staircase_next_ssd(ladder[[stair_gid]], step_dir, stair_spec)
      }
