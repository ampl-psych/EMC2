...
make_trend <- function(par_names, cov_names = NULL, kernels, bases = NULL,
  ...
  for(i in 1:length(par_names)){
    ...
    if(!kernels[i] %in% names(trend_help(kernels[i], return_types = TRUE)$kernels)){
      ...
      if(kernels[i] %in% c('delta', 'delta2kernel', 'delta2lr')) trend$at <- at
    }
...
run_kernel <- function(trend_pars = NULL, kernel, input, funptr = NULL) {
  ...
  for (j in seq_len(ncol(input))) {
    ...
    contrib <- switch(kernel,
      ...
      delta2lr = {
        good <- !is.na(covariate)
        tmp <- run_delta2lr(trend_pars[good,1], trend_pars[good,2], trend_pars[good,3], covariate[good])
        z <- rep(0, n); z[good] <- tmp; z
      },
      delta2kernel = {
        good <- !is.na(covariate)
        tmp <- run_delta2kernel(trend_pars[good,1], trend_pars[good,2], trend_pars[good,3], trend_pars[good,4], covariate[good])
        z <- rep(0, n); z[good] <- tmp; z
      }
    )
    contrib[is.na(contrib)] <- 0
...
run_delta2lr <- function(q0,alphaPos,alphaNeg,covariate) {
  ...
  for (i in 2:length(q)) {
    if(is.na(q0[i])) {
      pe[i-1] <- covariate[i-1]-q[i-1]
      alpha <- ifelse(pe[i-1]>0, alphaPos, alphaNeg)
