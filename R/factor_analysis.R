...
align_loadings <- function(emc = NULL, lambda = NULL, n_cores = 1, verbose = TRUE, rotate_fun = NULL) {
  ...
  while ((criterion == TRUE) & (totalIterations < maxIter)) {
    ...
    sp_res <- parallel::mclapply(
      X = 1:mcmcIterations, FUN = sp_new, lambda_varimax, q, p, dim_all_c, all_c, lambda_hat,
      st, cost.matrix, perm, mc.cores = n_cores
    )
    min_costs <- sapply(sp_res, FUN = function(x) {
      return(x$min_cost)
    })
    v_vectors <- do.call(rbind, lapply(sp_res, FUN = function(x) {
      return(x$min_perm)
    }))
    c_vectors <- do.call(rbind, lapply(sp_res, FUN = function(x) {
      return(x$min_c)
    }))
    objective <- sum(min_costs)
    lambda_hat_new <- Reduce("+", lapply(sp_res, FUN = function(x) {
      return(x$switchedMatrix)
    }))
    ...
    if (verbose) {
      cat(paste0("   -----  objective function = ", round(
        objective,
        3
      ), " -----"), "\n")
      cat("\n")
  ...
  for (i in 1:mcmcIterations) {
    lambda <- matrix(lambda_varimax[, , i], nrow = p, ncol = q)
    switchedMatrix <- matrix(c_vectors[i, ],
      ncol = q, nrow = p,
      byrow = T
    ) * lambda[, v_vectors[i, ]]
...
plot_relations <- function(emc = NULL, stage = "sample", plot_cred = FALSE,
  ...
  if (plot_cred || only_cred) {
    ...
    if (do_corr) {
      conf[seq(1, length(conf), ncol(means) + 1)] <- ""
    }
    if (only_cred) {
  ...
  if (do_corr) {
    ...
    corr_coef_col <- addCoef.col
    if (plot_means) corr_coef_col <- NULL

    if (plot_means) {
      labels <- format(round(means_raw, 2), nsmall = 2)
      if (only_cred) labels[is.na(means)] <- ""
      if (do_corr) labels[seq(1, length(labels), ncol(means) + 1)] <- ""
    }
    ...
    if (only_cred) {
      ...
      means[is.na(means)] <- 0
      corrplot(means,
        addCoef.col = corr_coef_col, number.cex = .75, tl.col = "black",
        p.mat = p.mat, insig = "blank", sig.level = 0.05
      )
    } else {
      corrplot(means, addCoef.col = corr_coef_col, number.cex = .75, tl.col = "black")
    }

    if (plot_means) {
      xs <- row(means)
      ys <- (ncol(means) + 1) - col(means)
      text(xs, ys, labels, col = addCoef.col, cex = 0.75, font = 2)
    }
  } else {
    ...
    if (only_cred) {
      ...
      p <- corrplot(means,
        is.corr = F, col = cols, col.lim = c(-1, 1),
...
factor_diagram <- function(emc = NULL, stage = "sample",
  ...
  fa.diagram(means,
    cut = cut, simple = simple, sort = sort, adj = adj,
    main = main, cex = cex
  )
}
...
make_SEM_diagram <- function(emc,
                             ...
                             cut = NULL,
                             ...) {
  get_matrix_vals <- function(vals, mat, i) {
    nm <- colnames(mat)[i]
    if (!is.null(nm) && !is.null(names(vals)) && nm %in% names(vals)) {
      return(vals[[nm]])
    }
    vals[[i]]
  }
  ...
  all_shapes <- c(
    rep("circle", n_pars + n_factors),
    rep("square", n_cov)
  )
  n <- length(all_shapes)
  ndf <- DiagrammeR::create_node_df(
    n = n, shape = all_shapes, fontsize = 14, fixedsize = F,
  ...
  if (plot_values) {
    ...
    if (any(K_mat != 0)) K_vals <- credint(emc, selection = "regressors", remove_constants = FALSE, digits = 2)
    if (any(G_mat != 0)) G_vals <- credint(emc, selection = "factor_regressors", remove_constants = FALSE, digits = 2)
  }

  for (i in 1:ncol(Lambda_mat)) {
    if (plot_values) cur_L_vals <- get_matrix_vals(L_vals, Lambda_mat, i)
    is_free <- Lambda_mat[, i] != 0
    if (cred_only) {
      is_free <- is_free & apply(apply(cur_L_vals, 1, sign), 2, FUN = function(x) {
        return(length(unique(x)))
      }) == 1
    }
    if (!is.null(cut)) {
      is_free <- is_free & (abs(cur_L_vals[, 2]) > cut)
    }
    from <- c(from, rep(n_pars + i, sum(is_free)))
    to <- c(to, which(is_free))
    if (plot_values) {
      label <- c(label, cur_L_vals[, 2][is_free])
    } else {
  ...
  if (any(B_mat != 0)) {
    for (i in 1:ncol(B_mat)) {
      if (plot_values) cur_B_vals <- get_matrix_vals(B_vals, B_mat, i)
      is_free <- B_mat[, i] != 0
      if (cred_only) {
        is_free <- is_free &
          apply(apply(cur_B_vals, 1, sign), 2, FUN = function(x) {
            return(length(unique(x)))
          }) == 1
      }
      from <- c(from, rep(n_pars + i, sum(is_free)))
      ...
      if (plot_values) {
        label <- c(label, cur_B_vals[, 2][is_free])
      } else {
        label_tmp <- rep("*", sum(is_free))
  ...
  if (any(K_mat != 0)) {
    for (i in 1:ncol(K_mat)) {
      if (plot_values) cur_K_vals <- get_matrix_vals(K_vals, K_mat, i)
      is_free <- K_mat[, i] != 0
      if (cred_only) {
        is_free <- is_free &
          apply(apply(cur_K_vals, 1, sign), 2, FUN = function(x) {
            return(length(unique(x)))
          }) == 1
      }
      ...
      to <- c(to, which(is_free))
      if (plot_values) {
        label <- c(label, cur_K_vals[, 2][is_free])
      } else {
        label_tmp <- rep("*", sum(is_free))
  ...
  if (any(G_mat != 0)) {
    for (i in 1:ncol(G_mat)) {
      if (plot_values) cur_G_vals <- get_matrix_vals(G_vals, G_mat, i)
      is_free <- G_mat[, i] != 0
      if (cred_only) {
        is_free <- is_free &
          apply(apply(cur_G_vals, 1, sign), 2, FUN = function(x) {
            return(length(unique(x)))
          }) == 1
      }
      from <- c(from, rep(n_pars + n_factors + i, sum(is_free)))
      to <- c(to, n_pars + which(is_free))
      if (plot_values) {
        label <- c(label, cur_G_vals[, 2][is_free])
      } else {
        label_tmp <- rep("*", sum(is_free))
...
rotater <- function(L_array, rot_fun, sign_convention = TRUE) {
  ...
  Abar_rot <- if (!is.null(fit$loadings)) {
    unclass(fit$loadings)
  } else if (!is.null(fit$L)) {
    unclass(fit$L)
  } else {
    stop("Rotation result missing $loadings/$L.")
  }
  Tmat <- if (!is.null(fit$Th)) {
    fit$Th
  } else if (!is.null(fit$Tmat)) {
    fit$Tmat
  } else if (!is.null(fit$rotmat)) {
    fit$rotmat
  } else {
