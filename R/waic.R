...
waic_subject <- function(emc, stage="sample", filter=0, subject){
  ...
  ll_mat <- t(calc_ll_manager(proposals, emc[[1]]$data[[subject]], emc[[1]]$model, return_trialwise=TRUE))
...
waic_pooled <- function(emc, stage="sample", filter=0){
  ...
  ll_list <- lapply(names(alpha), function(sub){
    ...
    t(calc_ll_manager(proposals, data[[sub]], model, return_trialwise=TRUE))  # [n_iter x n_trials_sub]
  })
