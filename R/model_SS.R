...
ST_staircase_function <- function(dts, staircase) {
  ...
  if (is.null(staircase$accST)) { # Indices for accumulator with SSD
    iSSD <- 1
  } else { # Only stop accumulator
    iSSD <- c(1, staircase$accST)
  } # NB: accST refers to rows in dts
  for (i in 1:ns) {
    if (SSD[i] < staircase$stairmin) SSD[i] <- staircase$stairmin
    if (SSD[i] > staircase$stairmax) SSD[i] <- staircase$stairmax
...
#      and can still have a stop-triggered response on a go-failure trial.
{
  ...
  if (any(isGO)) {
    dt[-1, ][isGO] <- rexG(
      ngo, pars[isGO, "mu"], pars[isGO, "sigma"],
      pars[isGO, "tau"]
    )
  }
  ...
  if (any(stair)) {
    ...
    out <- cbind.data.frame(
      R = factor(allR, levels = 1:nacc, labels = levels(lR)),
      rt = allrt, SSD = allSSD
    )
    return(out)
  }
...
pstopEXG <- function(parstop, n_acc, upper = Inf,
  ...
  ups <- sapply(1:sum(uniq), function(i) {
    my.integrate(
      f = stopfn_exg, lower = -Inf, SSD = SSDs[i], upper = upper[i],
...
pstopHybrid <- function(
  ...
  ups <- sapply(1:sum(uniq), function(i) {
    my.integrate(
      f = stopfn_rdex, lower = 0, upper = upper[i],
...
SShybrid <- function() {
  list(
    ...
    bound = list(
      minmax = cbind(
        v = c(1e-3, Inf), B = c(0, Inf), A = c(1e-4, Inf), t0 = c(0.05, Inf),
...
log_likelihood_race_ss <- function(pars, dadm, model, min_ll = log(1e-10)) {
  ...
  if (any(ispNR)) { # Note by definition no ST present
    ...
    if (any(ispstopNR)) { # Stop trial probability
      ...
      pStop <- pmin(1, pmax(
        0, # protection to keep in 0-1
        model$sfun(
          pars[ispNR & ispStop & ispGOacc, , drop = FALSE],
          n_acc = n_accG
        )
      ))
    ...
    if (any(ispStop)) { # Stop trials
      ...
      if (any(ispSGO)) {
        ...
        if (n_accG > 1) { # Looser survivor gp accumulators
          ...
          like[tGO] <- like[tGO] + apply(matrix(
            log(1 - model$pfunG(
              rt = dadm$rt[ispGOloss], pars = pars[ispGOloss, , drop = FALSE]
            )),
            nrow = n_accG - 1
      ...
      if (any(ispSST)) { # Go triggered 1a) with (1-pStop), all race,
        ...
        pStop <- model$sfun(pars[ispSST & ispGOacc, , drop = FALSE],
          n_acc = n_accG,
