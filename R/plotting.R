...
#' @param subject Subject number to plot. If NULL, plots all subjects separately.
...
#' @param layout Optional integer vector of length 2 (e.g. `c(2, 3)`) to override the automatic mfrow layout when plotting all subjects.
...
plot_trend <- function(input_data, emc, par_name, subject = NULL,
  ...
  if (!is.null(subject)) {
    if (!subject %in% all_subjects) {
      stop(sprintf( "Subject '%s' not found in the data. Available subjects: %s",
...
# Internal function — plots trend for a single subject
.plot_trend_single <- function(input_data, emc, par_name, subject,
                               filter = NULL, on_x_axis = 'trials',
                               pp_shaded = TRUE, ...) {
  ...
  if (!is.list(input_data)) {
    ...
    updated <- lapply(seq_along(input_data), function(i)
      data.frame(input_data[[i]][row_filter, par_name, drop = FALSE],
                 x = dadm[row_filter, on_x_axis]))
    credible_interval <- aggregate(.~x, do.call(rbind, updated), quantile, c(0.025, .5, .975))
    ...
    if (pp_shaded) {
      ...
      ylim <- range(sapply(updated, function(u) range(u[, par_name])))
    }
  }
  ...
  full_args <- c(list(x = x, y = trend, type = 'l', ylab = par_name), dots)
  do.call(plot, full_args)

  if (!is.null(credible_interval)) {
    if (pp_shaded) {
      polygon(x = c(x, rev(x)),
              y = c(credible_interval[[2]][, 1], rev(credible_interval[[2]][, 3])),
              col = adjustcolor(3, alpha.f = .3), border = adjustcolor(1, alpha.f = .4))
    } else {
