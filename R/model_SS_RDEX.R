...
SSrdexABCD <- function() {
    list(
    type="RACE",
    p_types=c("v","B","A","t0","s","muS","sigmaS","tauS","tf","gf","v0","ta"),
    # Transform to natural scale
    Ntransform=function(x) {
      # transform parameters back to real line
      isprobit <- dimnames(x)[[2]] %in% c("tf","gf")
    ...
    Ttransform = function(pars,dadm) {
      ...
      ass <- pars[,"SSD"] >= pars[,"ta"] # asymptotic discrimination
      pars[!ass,"v"] <- pars[!ass,"v0"] +
        (pars[!ass,"v"]-pars[!ass,"v0"])*pars[!ass,"SSD"]/pars[!ass,"ta"]

      attr(pars,"ok") <- (pars[,"t0"] > .05) & ((pars[,"A"] > 1e-6) | pars[,"A"] == 0) &
        (pars[,"tauS"] < 1) & (pars[,"sigmaS"] < 1) & (pars[,"ta"] > 1e-3)
