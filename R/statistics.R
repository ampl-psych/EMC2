...
compare_subject <- function(
  ...
  if (!is.null(subject)) {
    if (is.integer(subject)) {
      subjects <- subjects[subject]
    } else {
      if (is.character(subject)) {
        if (!all(subject %in% subjects)) {
          stop("subject name(s) not in subjects")
        }
        subjects <- subject
      }
    }
  ...
  if (print_summary) {
    ...
    print(
      list(
        DIC = round(pDIC, digits),
        BPIC = round(pBPIC, digits)
      )
    )
    mnams <- unlist(
      lapply(
