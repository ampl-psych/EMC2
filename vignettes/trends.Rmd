...
their sum is pinned down. `centre = TRUE` subtracts the mean of `k(c)` over the
rows the kernel applies to, which makes `p` the value at the *average*
covariate and largely removes the correlation. It changes what `p` means, so
adjust its prior accordingly.

```{r, eval=FALSE}
kern  <- make_kernel(cov_names = "SSD", type = "sat_incr", centre = TRUE)
