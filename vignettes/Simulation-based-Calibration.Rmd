...
Before we start with the SBC, we want to give a quick note on data compression. By default compresses RTs to bins of 20ms, since typical lab set-ups do not allow response times to be measured with a higher resolution. However, generated data is not subject to the same physical constraints. Therefore, and to avoid biases, compression is turned off. To turn on compression regardless, you can set `compression = TRUE`, and set the resolution using `rt_resolution = 0.02 (for example)` in the `run_sbc` function, which will speed up computation but potentially introduces biases.
...
Next we can simply run `run_sbc` with our specified design and prior. We can use `?run_sbc` to see the description of the function arguments. The trials argument refers the number of trials per cell, since we have an S factor in our design (and all other designs used), we will simulate 200 trials per data set.
...
Next we test non-hierarchical for the DDM as well. Initial results actually showed bias in the estimation of non-decision time and non-decision time variability. Whereas we first based our DDM implementation on the *rtdists* package, the bias pushed us to test with alternative packages, after which we stumbled on the *WienR* package. Which not only boasts much faster likelihood evaluations, but also no longer shows bias in the SBC results. Hence in the newest CRAN release we adapted our C++ code to instead use the *WienR* *C++* routines.
...
SBC_DDM_single <- run_sbc(design_DDM, prior_DDM, replicates = 500, trials = 100, 
                          fileName = "WienR_single.RData", cores_per_chain = 30)
...
As you can see minor misfits in *a* and *sv* but nothing too worrying. To double check we did refits, that showed that this bias is not systematic.
...
Now we dive into hierarchical SBC for the LBA model. The prior structure we chose was as follows. Here we have to be extra mindful of choosing a prior that ensures plausible generated data, since there's a double propagation of prior space. First through the prior on the standard deviation of the group-level mean, but also through the prior on the group-level variance.
...
SBC_LBA <- run_sbc(design_LBA, prior_LBA, replicates = 500, trials = 100,
                        n_subjects = n_subjects, fileName = "SBC_LBA.RData", cores_per_chain = 30)
```
...
```{r, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
design_DDM <- design(factors=list(subjects=1:30,S=c("left", "right")),
                       Rlevels = c("left", "right"),
                       formula =list(v~1,a~1, t0~1, s~1, Z~1, sv~1, SZ~1, st0 ~ 1),
                       constants=c(s=log(1)),
                       model = DDM)
...
In contrast to the other models, we generated 200 (instead of 100) trials per person per cell, since with 100 trials the model sometimes did not converge or was very slow to converge.

```{r, eval=FALSE}
SBC_DDM <- run_sbc(design_DDM, prior_DDM, replicates = 500, trials = 200,
                        n_subjects = n_subjects, fileName = "SBC_DDM.RData", cores_per_chain = 30)
```

### RDM

For completion and as a proof of correct implementation we also test hierarchical SBC for the RDM with the following model definition and prior structure.
...
Also for the LNR hierarchical SBC shows no considerable bias
