...
design_base <- design(factors = list(subjects = 1, S = 1:2),
                    Rlevels = 1:2,
                    covariates = list(covariate1 = covariate1, covariate2 = covariate2),
                    matchfun = matchfun,
                    trend = trend,
...
p_vector <- sampled_p_vector(design_base, doMap = FALSE)
p_vector[1:6] <- c(-1, 1.5, log(1), log(.2), log(.2), log(.2))

dat <- make_data(p_vector, design_base, n_trials = n_trials)

LNR2cov <- make_emc(dat, design_base, compress = F, n_chains = 1)
...
LNR2cov_shared <- make_emc(dat, design_base_shared, compress = FALSE, n_chains = 1)
...
LNR_premap <- make_emc(dat, design_premap, compress = FALSE, n_chains = 1)
...
LNR_pretrans <- make_emc(dat, design_pretrans, compress = FALSE, n_chains = 1)
...
LNR_posttrans <- make_emc(dat, design_posttrans, compress = FALSE, n_chains = 1)
...
LNR_bases <- make_emc(dat, design_bases, compress = FALSE, n_chains = 1)
...
LNR_poly <- make_emc(dat, design_poly, compress = FALSE, n_chains = 1)
...
LNR_shared_premap <- make_emc(dat, design_shared_premap, compress = FALSE, n_chains = 1)
...
LNR_shared_posttransform <- make_emc(dat, design_shared_posttransform, compress = FALSE, n_chains = 1)
