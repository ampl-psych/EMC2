...
# specify design
design_LBABE <- make_design(data = forstmann,model=LBA,
...
samplers <- make_samplers(forstmann,design_LBABE,type="standard",  prior=prior_LBABE,
                          compress = FALSE)

test_that("make_samplers", {
  expect_snapshot(str(samplers, give.attr = F))
})
