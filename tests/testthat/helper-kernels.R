...
beta_binomial_basic <- function(x, a0, b0) {
  n_total <- length(x)
  out <- numeric(n_total)
  n_hit <- 0
  n_trial <- 0
...
beta_binomial_decay <- function(x, a0, b0, decay) {
  n_total <- length(x)
  out <- numeric(n_total)
  n_hit <- 0
  n_trial <- 0
...
beta_binomial_window <- function(x, a0, b0, window) {
  n_total <- length(x)
  out <- numeric(n_total)
  n_hit <- 0
  n_trial <- 0
  ...
  for (t in seq_len(n_total)) {
    while (length(buf[["obs"]]) > 0 && (t - buf[["idx"]][1]) > window) {
...
#   }
#
#   if (length(covariate) > length(a0) && length(a0) == 1L) {
#     a0 <- rep(a0, length(covariate))
#   }
#   if (length(covariate) > length(b0) && length(b0) == 1L) {
#     b0 <- rep(b0, length(covariate))
#   }
#
