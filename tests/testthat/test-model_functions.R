...
design_LBA <- make_design(data = dat,model=LBA,matchfun=matchfun,
...
design_RDM <- make_design(data = dat,model=RDM,matchfun=matchfun,
...
# Also DDM
design_DDM <- make_design(data = dat,model=DDM,
...
LNR_s <- make_samplers(dat, design_LNR, compress = F, n_chains = 1)
LBA_s <- make_samplers(dat, design_LBA, compress = F, n_chains = 1)
...
test_that("LNR", {
  ...
  set.seed(123)
  expect_snapshot(init_chains(LNR_s, particles = 10, cores_per_chain = 4)[[1]]$samples)
...
test_that("LBA", {
  RNGkind("L'Ecuyer-CMRG")
  set.seed(123)
  expect_snapshot(init_chains(LBA_s, particles = 10, cores_per_chain = 4)[[1]]$samples)
...
test_that("RDM", {
  RNGkind("L'Ecuyer-CMRG")
  set.seed(123)
  expect_snapshot(init_chains(RDM_s, particles = 10, cores_per_chain = 4)[[1]]$samples)
...
test_that("DDM", {
  RNGkind("L'Ecuyer-CMRG")
  set.seed(123)
  expect_snapshot(init_chains(DDM_s, particles = 10, cores_per_chain = 4)[[1]]$samples)
