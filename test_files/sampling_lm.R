...
sample_store_lm <- function(data, pars_fixed, pars_random, g_fixed, g_random,  subjects, iters = 1, stage = "init") {
  ...
  components <- c(subjects, "between_subs")
...
make_dadm_lm <- function(data, DM_fixed, DM_random, full_data){
  s <- data$subjects[1]
  full_data <- full_data[attr(full_data, "expand"),]
  for(i in 1:length(DM_random)){
    curr_DM <- DM_random[[i]]
    if(!is.null(curr_DM)){
      curr_DM <- curr_DM[full_data$subject == s,get_sub_idx(s, colnames(curr_DM))]
      DM_random[[i]] <- curr_DM
    }
    DM_fixed[[i]] <- DM_fixed[[i]][full_data$subject == s,]
  }
...
pmwgs_lm <- function(dadm, pars = NULL, ll_func = NULL, prior = NULL, type, ...) {
  ...
  pars_fixed <- unlist(lapply(DM_fixed, colnames))
  pars_random <- unlist(lapply(DM_random, colnames))
  ...
  split_dadm <- split(dadm[[1]], dadm[[1]]$subjects)
  split_dadm <- lapply(split_dadm, make_dadm_lm, DM_fixed, DM_random, dadm[[1]])
  # Storage for the samples.
  samples <- sample_store_lm(dadm, pars_fixed, pars_random, g_fixed, g_random, subjects)
  sampler <- list(
    data = split_dadm,
    dm_random = DM_random,
    dm_fixed = DM_fixed,
...
get_startpoints_lm <- function(pmwgs){
  ...
  mu_random <- rep(1, length(pmwgs$pars_random))
  return(list(mu_fixed = mu_fixed, var_fixed = var_fixed,
              mu_random = mu_random, var_random = var_random))
}


start_proposals_between <- function(n_particles, pmwgs){
  #Draw the first start point
  group_mean <- c(pmwgs$mu_fixed, pmwgs$mu_random[!grepl("subjects", pmwgs$pars_random)])
  group_var <- c(pmwgs$var_fixed, pmwgs$var_random[!grepl("subjects", pmwgs$pars_random)])

  proposals <- particle_draws(n_particles, group_mean, group_var)
  colnames(proposals) <- c(pmwgs$pars_fixed, pmwgs$pars_random[grepl("subjects", pmwgs$pars_random)])
  lws <- parallel::mclapply(1:n_subjects, calc_ll_for_group, proposals, data, likelihood_func, subjects, mc.cores = n_cores)
  lw <- Reduce("+", lws)
...
init_ES <- function(pmwgs, verbose = FALSE, particles_random = 1000,
  ...
  if(is.null(epsilon)) epsilon <- rep(0.5, pmwgs$n_subjects + 1)
  if(length(epsilon) == 1) epsilon <- rep(epsilon, pmwgs$n_subjects + 1)
  proposals_fixed <- start_proposals_between()
  proposals_within <- parallel::mclapply(X=1:pmwgs$n_subjects,FUN=start_proposals_within,
                                  parameters = startpoints, n_particles = particles,
                                  pmwgs = pmwgs,mc.cores = n_cores)
  proposals_random <- array(unlist(proposals), dim = c(pmwgs$n_pars  + 2, pmwgs$n_subjects))
