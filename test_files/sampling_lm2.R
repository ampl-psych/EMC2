...
new_particle_within <- function(subj_mean, data, epsilon, chains_cov, eff_props, n_particles, hyper, pars_between, ll_func,
  ...
  red_name <- gsub(paste0("_subjects", data$subjects[1]), "", names(subj_mean))
  pars_between_old <- length(red_name)
  for(i in 1:length(subj_mean)){
    idx <- which(red_name[i] == names(pars_between))
    # subj_mean_prior[idx] <- subj_mean[idx]  + pars_between[idx]
    group_mean[idx]  <- group_mean[idx]  + pars_between[idx]
    # eff_mean_sub[idx]  <- eff_mean_sub[idx] + pars_between[idx]
    pars_between_old[i] <- pars_between[idx]
    pars_between[idx] <- 0
  }
  is_nested <- names(pars_between) == red_name
  ...
  return(list(proposal = proposal_out - pars_between_old, ll = mean(out_lls), origin = origin))
}
