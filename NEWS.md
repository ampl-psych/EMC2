...
- Fixed failures when `group_design()` is used during sampling or model comparison.
- Applied parameter bounds consistently to trend parameters and prevented a degenerate RDM parameter region.

## New features

- Added multinomial and ordered choice-response models, with `plot_fit_choice()` diagnostics.
- Extended `group_design()` support, including nested random-effects specifications.
- Extended trends with custom kernels, covariate maps, parameter inputs, latent-stream outputs, and improved `plot_trend()` support.
- Added `add_ICs_MLL()` to retain information criteria and marginal likelihoods; DIC/BPIC now defaults to the conditional form.
- Added graphical-model support with `graphical_model()`.
