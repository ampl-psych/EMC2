// SOURCE OF TRUTH: ~/Downloads/julian/nle/handover/nle-methods/port/src/flow_ddm.cpp
// Verbatim copy (below this header); sha256 of the source file: 0a9ea58e9416b40699271628b29be2472bbf86647d2fdadafc27fa9882bd560b
...
static void emc2_to_triple(const double* theta, int n, std::vector<double>& out) {
  out.assign(theta, theta + n);
...
static void ddm_condition(const DdmModel& m, const double* theta, DdmCond& c) {
  ...
  if (m.triple) {
    ...
    emc2_to_triple(theta, m.n_ctx, tf);
    for (int j = 0; j < m.n_ctx; ++j) {
      ctx[j]     = (tf[j]    - m.scaler_mean[j])     / m.scaler_scale[j];
      ctx_clf[j] = (theta[j] - m.clf_scaler_mean[j]) / m.clf_scaler_scale[j];
    }
  } else {
    for (int j = 0; j < m.n_ctx; ++j)
