...
// ---------------------------------------------------------------------------
// Stop-signal trial-loop likelihood (SSEXG / SSRDEX) on the cens framework.
//
// Per-trial SS log-likelihood: go trials, stop-fail (signal-respond),
// stop-success (withheld), stop-triggered (ST) accumulators, trigger failure
// (tf), go failure (gf), plus the cens censoring codes: deadline censoring (UC,
// missingness 2), lower censoring (LC, code 1), both (3) and intrinsic
// no-response (4). All integrals use the package's hcubature (ss_integrate.h).
//
// Column layout of `pars` is the model's p_types order (fixed indices, see
// resolve_ss_adapter); calc_ll materialises exactly those columns by name.
//
// Must be included AFTER model_SS_EXG.h / model_SS_RDEX.h (function pointers
// reference texg_go_lpdf / rdex_go_lpdf / ss_*_stop_success_lpdf) and after
...
// Stop-success integral accuracy (evaluation budget = max_subdiv * 64), matching
// the R reference (my.integrate: rel.tol = 1e-6, abs.tol = 1e-8).
constexpr int    SS_STOP_MAX_SUBDIV = 100;
constexpr double SS_STOP_ABS_TOL    = 1e-8;
constexpr double SS_STOP_REL_TOL    = 1e-6;
...
// Lower-censoring response-mass integral: the trial response density
// integrated over [lower, upper] (cens hcubature via ss_integrate.h).
...
static inline double ss_integrate_lc_response_mass(
  ...
  double res = ss_integrate(ss_lc_integrand, &data, lower, upper,
                            SS_STOP_ABS_TOL, SS_STOP_REL_TOL, /*max_eval=*/200000u);
...
// Writes the per-trial log-likelihood of the (compressed) dadm into ll_buf
// (length n_trials); bound checking, clamping, expansion and summation are
// done by the caller (calc_ll: apply_bounds + expand_clamp_sum), as for the
// other models. Non-participating rows (missingness 0) are not supported.
// ---------------------------------------------------------------------------
inline void c_log_likelihood_ss(
    NumericMatrix pars, DataFrame data, const int n_trials, double min_ll,
    ss_go_pdf_fn go_lpdf_ptr, ss_go_pdf_fn go_lccdf_ptr,
    ss_stop_surv_fn stop_logsurv_ptr, ss_stop_success_fn stop_success_ptr,
    int idx_tf, int idx_gf, double* ll_buf) {
  ...
  // non-observed trial (1=lower, 2=upper/deadline, 3=both; NA=observed). An
  // intrinsic no-response (successful stop / go failure) is code 2 with UC=Inf
  // when there is no deadline. rt is finite (observed) or NA, never Inf/-Inf.
  // Absent column => observed, with a fallback below that treats an NA rt as an
  // intrinsic no-response.
  ...
  for (int trial = 0; trial < n_trials; trial++) {
    ...
    bool stop_signal_presented = R_FINITE(SSD[start_row]);
    ...
    // intrinsic no-response, never an observed trial.
    if (miss == NA_INTEGER && !R_FINITE(rt)) miss = -1;

    // Intrinsic no-response (no deadline): stop-success or go-failure.
    ...
    // Upper (deadline) censoring log-mass (code 2) = P(NO response observed by
    // the deadline UC). On a stop trial the observer cannot tell a successful stop
    // from a too-slow (censored) response, so both are modelled together
    // (make_missing labels all such no-responses code 2):
    //   P = gf + (1-gf)*[ tf*S_go(UC) + (1-tf)*(P(stop wins by UC) + P(neither finished by UC)) ]
    //     = p(gofail) + p(Stop) + (1-p(Stop))*p(Censor).
    // With UC=Inf it reduces to the intrinsic no-response mass (== withheld_lp).
    auto upper_deadline_lp = [&]() -> double {
      // No (finite) deadline for this trial: the code-2 mass is the intrinsic
      // no-response mass (also covers UC supplied only as an attribute).
      if (!R_FINITE(uc)) return withheld_lp();
    ...
    if (miss == -1) { lls[trial] = withheld_lp(); continue; }
  ...
  // NaN / -Inf / < min_ll entries are clamped by the caller (expand_clamp_sum).
}

// Resolve function pointers + fixed column indices for SS (EXG or RDEX).
// `cols` is the parameter column layout the fixed indices assume (the model's
// p_types order); calc_ll materialises exactly these columns from the ParamTable.
...
static inline SSModelAdapter resolve_ss_adapter(const std::string& type_std) {
  SSModelAdapter a;
  const bool is_exg = (type_std == "SSEXG");
  ...
  a.idx_gf           = is_exg ? 7 : 9;
  a.cols = is_exg
    ? CharacterVector::create("mu", "sigma", "tau", "muS", "sigmaS", "tauS",
                              "tf", "gf", "exg_lb", "exgS_lb")
    : CharacterVector::create("v", "B", "A", "t0", "s", "muS", "sigmaS", "tauS",
                              "tf", "gf", "exgS_lb");
  return a;
}
