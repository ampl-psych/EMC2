...
// ----------------------------------------------------------------------------
// RACING-DIFFUSION GO / NORMAL STOP FUNCTIONS
// ----------------------------------------------------------------------------

static inline double rdex_tnormal_log_normalizer(double mu, double sigma, double lb) {
  if (sigma <= 0.0) return R_NegInf;
  if (lb == R_NegInf) return 0.0;
  return PNORM_STD((lb - mu) / sigma, false, true);
}

static inline double rdex_tnormal_log_pdf(double x, double mu, double sigma, double lb) {
  if (sigma <= 0.0 || x <= lb) return R_NegInf;
  double log_z = rdex_tnormal_log_normalizer(mu, sigma, lb);
  if (!R_FINITE(log_z)) return R_NegInf;
  double d = DNORM(x, mu, sigma);
  return (d > 0.0 && R_FINITE(d)) ? std::log(d) - log_z : R_NegInf;
}

static inline double rdex_tnormal_log_survivor(double q, double mu, double sigma, double lb) {
  if (sigma <= 0.0) return R_NegInf;
  if (q <= lb) return 0.0;
...
double ss_rdex_normal_stop_fail_lpdf(
    double RT,
    double SSD,
    NumericMatrix pars,
    LogicalVector winner,
  ...
  double go_lprob = ss_rdex_go_lpdf(RT, pars, winner, min_ll);
  double stop_survivor_lprob = rdex_tnormal_log_survivor(RT - SSD, pars(0, 5), pars(0, 6), pars(0, 9));
  if (!R_FINITE(stop_survivor_lprob)) stop_survivor_lprob = min_ll;
  return go_lprob + stop_survivor_lprob;
}

class rdex_normal_stop_success_integrand : public Func {
private:
  const double SSD;
  const double min_ll;
...
public:
  rdex_normal_stop_success_integrand(
    double SSD_,
    NumericMatrix pars_,
    double min_ll_
  ) :
  SSD(SSD_),
  min_ll(min_ll_),
  muS(pars_(0, 5)), sigmaS(pars_(0, 6)), normS_lb(pars_(0, 9)),
  n_go(pars_.nrow()),
  alpha(n_go), nu(n_go), gamma(n_go), t0(n_go)
  {
    for (int i = 0; i < n_go; ++i) {
  ...
  double operator()(const double& x) const {
    double log_d = rdex_tnormal_log_pdf(x, muS, sigmaS, normS_lb);
    if (!R_FINITE(log_d)) log_d = min_ll;
    double summed_log_s = 0.0;
    for (int i = 0; i < n_go; ++i) {
      double dt_i = (x + SSD) - t0[i];
...
static inline double ss_rdex_normal_stop_success_lpdf(
    double SSD,
    NumericMatrix pars,
    double min_ll,
    double upper = R_PosInf,
  ...
  rdex_normal_stop_success_integrand f(SSD, pars, min_ll);
  double err_est, res;
  int err_code;
  double muS = pars(0, 5);
  double sigmaS = pars(0, 6);
  double lb = pars(0, 9);
...
NumericVector ss_rdex_normal_lpdf(
    NumericVector RT,
    IntegerVector R,
    NumericVector SSD,
    NumericVector lR,
  ...
  // pars columns: v=0, B=1, A=2, t0=3, s=4, muS=5, sigmaS=6, tf=7, gf=8, normS_lb=9, b=10
  ...
  for (int trial = 0; trial < n_trials; trial++) {
    ...
    if (response_observed) {
      if (stop_signal_presented) {
        ...
        stop_fail_lprob = ss_rdex_normal_stop_fail_lpdf(
          RT[start_row],
          SSD[start_row],
          pars(Range(start_row, end_row), _),
          winner[Range(start_row, end_row)],
      ...
      if (stop_signal_presented) {
        ...
        if (mix_w <= 1e-6) {
          ...
          stop_success_integral = ss_rdex_normal_stop_success_lpdf(
            SSD[start_row],
            pars(Range(start_row, end_row), _),
            min_ll
          );
...
// ----------------------------------------------------------------------------
// RACING-DIFFUSION GO / WEIBULL STOP FUNCTIONS
// ----------------------------------------------------------------------------

static inline double rdex_weibull_log_pdf(double x, double shape, double scale) {
...
static inline double rdex_weibull_log_survivor(double q, double shape, double scale) {
  if (shape <= 0.0 || scale <= 0.0) return R_NegInf;
  if (q <= 0.0) return 0.0;
  return -std::pow(q / scale, shape);
}

double ss_rdex_weibull_stop_fail_lpdf(
    double RT,
    double SSD,
    NumericMatrix pars,
    LogicalVector winner,
  ...
  double go_lprob = ss_rdex_go_lpdf(RT, pars, winner, min_ll);
  double stop_survivor_lprob = rdex_weibull_log_survivor(RT - SSD, pars(0, 5), pars(0, 6));
  if (!R_FINITE(stop_survivor_lprob)) stop_survivor_lprob = min_ll;
  return go_lprob + stop_survivor_lprob;
}

class rdex_weibull_stop_success_integrand : public Func {
private:
  const double SSD;
  const double min_ll;
...
public:
  rdex_weibull_stop_success_integrand(
    double SSD_,
    NumericMatrix pars_,
    double min_ll_
  ) :
  SSD(SSD_),
  min_ll(min_ll_),
  shapeS(pars_(0, 5)), scaleS(pars_(0, 6)),
  n_go(pars_.nrow()),
  alpha(n_go), nu(n_go), gamma(n_go), t0(n_go)
  {
    for (int i = 0; i < n_go; ++i) {
  ...
  double operator()(const double& x) const {
    double log_d = rdex_weibull_log_pdf(x, shapeS, scaleS);
    if (!R_FINITE(log_d)) log_d = min_ll;
    double summed_log_s = 0.0;
    for (int i = 0; i < n_go; ++i) {
      double dt_i = (x + SSD) - t0[i];
...
static inline double ss_rdex_weibull_stop_success_lpdf(
    double SSD,
    NumericMatrix pars,
    double min_ll,
    double upper = R_PosInf,
  ...
  rdex_weibull_stop_success_integrand f(SSD, pars, min_ll);
  double err_est, res;
  int err_code;
  double shapeS = pars(0, 5);
  double scaleS = pars(0, 6);
...
NumericVector ss_rdex_weibull_lpdf(
    NumericVector RT,
    IntegerVector R,
    NumericVector SSD,
    NumericVector lR,
  ...
  // pars columns: v=0, B=1, A=2, t0=3, s=4, shapeS=5, scaleS=6, tf=7, gf=8, b=9
  ...
  for (int trial = 0; trial < n_trials; trial++) {
    ...
    if (response_observed) {
      if (stop_signal_presented) {
        ...
        stop_fail_lprob = ss_rdex_weibull_stop_fail_lpdf(
          RT[start_row],
          SSD[start_row],
          pars(Range(start_row, end_row), _),
          winner[Range(start_row, end_row)],
      ...
      if (stop_signal_presented) {
        ...
        if (mix_w <= 1e-6) {
          ...
          stop_success_integral = ss_rdex_weibull_stop_success_lpdf(
            SSD[start_row],
            pars(Range(start_row, end_row), _),
            min_ll
          );
